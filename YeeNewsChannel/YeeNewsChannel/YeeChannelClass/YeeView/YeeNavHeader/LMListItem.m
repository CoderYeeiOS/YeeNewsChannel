    ...
    if (!self.hiddenBtn.hidden) {
        if (self.location == top) {
            ...
            if (self.operationBlock) {
                self.operationBlock(topViewClick,self.channeModel);
            }
    ...
    switch (pan.state) {
        ...
        case UIGestureRecognizerStateChanged:{
            ...
            if (InTopView) {
                ...
                if (self.operationBlock) {
                    self.operationBlock(FromTopToTop,self.channeModel);
                }
            }
                ...
                [self animationForTopView];
                if (self.operationBlock) {
                    self.operationBlock(FromTopToTopLast,self.channeModel);
                }
            }
...
-(void)refreshCollectionView
{
    ...
    {
        [self.topTitleArray addObject:item.channeModel];
    }
...
-(void)changeFromTopToBottom
{
    ...
    [self removeGestureRecognizer:self.gesture];
    if (self.operationBlock) {
        self.operationBlock(FromTopToBottomHead,self.channeModel);
    }
    [self animationForWholeView];
}
...
-(void)changeFromBottomToTop
{
    ...
    self.location = top;
    if (self.operationBlock) {
        self.operationBlock(FromBottomToTopLast,self.channeModel);
    }
    [self animationForWholeView];
}
